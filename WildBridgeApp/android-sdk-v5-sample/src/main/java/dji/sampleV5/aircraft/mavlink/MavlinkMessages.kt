...
internal object MavlinkMessages {
     ...
     *
     * The standard gimbal v2 attitude report, which WildBridge is already entitled to send: the
     * camera component advertises the gimbal capability, and a ground station that understands
     * gimbals reads the pointing direction from here rather than from anything WildBridge-specific.
     *
     ...
     * without knowing anything about WildBridge. The geo-referenced target point it computes has
     * no standard home and travels in WILDBRIDGE_STATUS instead.
     */
    ...
    /**
     * WildBridge's residue. Field order and offsets come from wildbridge.xml as generated by
     * mavgen — see GroundStation/mavlink/wildbridge.xml, which is the source of truth for both
     * ends and for the CRC_EXTRA of 127.
     */
    fun wildbridgeStatus(snapshot: MavlinkSnapshot, timeBootMs: Long): ByteArray {
        var flags = 0
        if (snapshot.manualOverrideActive) flags = flags or WB_FLAG_MANUAL_OVERRIDE
        if (snapshot.readyToTakeoff) flags = flags or WB_FLAG_READY_TO_TAKEOFF
        if (snapshot.homeSet) flags = flags or WB_FLAG_HOME_SET
    ...
    /** WildBridge has one gimbal; 0 means "the only one". */
    ...
    /** WILDBRIDGE_STATUS_FLAGS, from wildbridge.xml. */
    private const val WB_FLAG_MANUAL_OVERRIDE = 1
